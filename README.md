...
- Load the necessary Shapefile datasets in the QGIS layers panel. The layers are:

    * **Empty Linkage /MODFLOW Grid**: An empty linkage file (if this is what is wanted, otherwise check "Get the input linkage..." and upload the MODFLOW model).
    * **WEAP Nodes**: WEAP model schematic nodes.
    * **WEAP Reaches**: WEAP model reaches (rivers/canals).
    * **Demand Sites**: Additional demand areas.
...
- Select each layer that contains Catchment and Groundwater information, and the **name column** that identifies each polygon.
